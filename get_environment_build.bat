...
set "PY_VER="
for /f "tokens=2 delims= " %%V in ('".venv\Scripts\python.exe" -V 2^>^&1') do set "PY_VER=%%V"
...
if not defined PY_TAG (
  echo Failed to detect Python version from .venv
  exit /b 1
)
...
echo Updated requirements.txt from local .venv
